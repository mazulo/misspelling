...
def convert_into_json():
    ...
    with open(filename, encoding="utf-8") as wikipedia_file:
        for line in wikipedia_file.readlines():
            wrong_word, right_words = line.split("->")
            list_right_words = []
            if "," in right_words:
                list_right_words = list(right_words.split(","))
            corrections = []
            if list_right_words:
    ...
    with open(filename, "w", encoding="utf-8") as wikipedia_file:
        ordered_dict = OrderedDict(sorted(word_dict.items(), key=lambda t: t[0]))
